...
- `/sessions`, `/stop`, `/prune` 명령으로 상태 조회/정리

## 주의

- 웹 접속 시마다 `tmux attach-session` 을 붙이는 PTY 하나가 새로 생성됩니다.
- `omx` 는 `--madmax` 로 실행됩니다.
- `omc` 는 기본적으로 `claude --dangerously-skip-permissions` 로 실행되며, 기존 Claude auth/home 상태를 그대로 사용합니다. 필요하면 `OMC_CLI_ENTRY`로 별도 엔트리를 지정할 수 있습니다.
